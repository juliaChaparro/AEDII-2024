...
void* acessar_inicio_lse(t_lse* lse){
    return lse->prim->carga_util;
}

void* proximo_elem_lse(t_lse* lse, void* elem){
    t_elemento_lse* cam = lse->prim;

    while (cam != NULL && cam->carga_util != elem) {
        cam = cam->prox;
    }

    if (cam != NULL && cam->prox != NULL) {
